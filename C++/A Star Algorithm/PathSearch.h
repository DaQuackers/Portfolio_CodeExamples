...
namespace fullsail_ai { namespace algorithms {
	...
	struct PlannerNode
	{
		PlannerNode* parent;
		SearchNode* ActNode;
		float givenCost, finalCost;
		bool operator>(PlannerNode* rhs)
		{
	...
	{
		TileMap* themap;
		SearchNode* Start, *Goal, *succ;
		PlannerNode* curr, *node;
		float tempcost;
		std::map<Tile*, SearchNode*> FullList;
		std::map<SearchNode*, PlannerNode*> VisitedList;
		PriorityQueue<PlannerNode*> que2;
	public:
